...
typedef struct _aladdin_traffic_t {
  // Pointer to the start address.
  void* start_addr;
  // Size of the traffic.
  size_t size;
  // Address of the finish flag.
  void* finish_flag;
  // Is traffic all writes?
  bool is_write_traffic;
  // Is ACP or DMA traffic?
  bool is_acp_traffic;
} aladdin_traffic_t;
...
/* Schedules the  traffic generator, which can generate DMA/ACP traffic.
 */
void invokeAladdinTrafficGenAndBlock(void* addr,
                                     size_t size,
                                     bool is_write_traffic,
                                     bool is_acp_traffic);
