...
const char USE_DEFAULT_OFFLOAD[] = "USE_DEFAULT_OFFLOAD";
const char DMA_ALWAYS[] = "DMA_ALWAYS";
const char ACP_ALWAYS[] = "ACP_ALWAYS";
const char DYNAMIC_DMA_ACP[] = "DYNAMIC_DMA_ACP";
...
int validate_data_mvmt_policy(cfg_t* cfg, cfg_opt_t* opt) {
    ...
    if (strcmp(value, DMA_ALWAYS) != 0 &&
        ...
        strcmp(value, DYNAMIC_DMA_ACP) != 0 &&
        strcmp(value, USE_DEFAULT_OFFLOAD) != 0) {
...
data_mvmt_policy str2policy(const char* value) {
    ...
    if (strcmp(value, ACP_ALWAYS) == 0)
        return AcpAlways;
    if (strcmp(value, DYNAMIC_DMA_ACP) == 0)
        return DynamicDmaAcp;
    if (strcmp(value, USE_DEFAULT_OFFLOAD) == 0)
        return UseDefaultOffload;
...
static void read_device_parameters(cfg_t* all_opts, device_t* device) {
    if (cfg_size(all_opts, "device") != 0) {
        ...
        device->use_sw_prefetch = cfg_getbool(device_opts, "use_sw_prefetch");
        ...
        device->inputs_load_policy =
                str2policy(cfg_getstr(device_opts, "inputs_load_policy"));
    } else {
        ...
        device->use_sw_prefetch = false;
        device->umem_size = 0;
        device->spad_size = 0;
        device->l2_size = 0;
        device->weights_load_policy = DmaAlways;
        device->inputs_load_policy = UseDefaultOffload;
    }
}
...
static void print_device_config(device_t* device) {
    ...
    printf("CPU offload mechanisms:\n"
           ...
           bool_to_yesno(device->use_sw_prefetch));
}
...
int configure_network_from_file(const char* cfg_file,
    ...
    *sampling_ptr = (sampling_param_t*) malloc_aligned(sizeof(sampling_param_t));
    ...
    if ((*device_ptr)->use_sw_prefetch && NUM_WORKER_THREADS == 0) {
        printf("[WARNING]: SW prefetching of data is enabled, but there are no "
               "worker threads to do the work! You must specify the argument "
               "--num-threads=N.\n");
    }
    cfg_free(all_opts);
    return num_layers;
