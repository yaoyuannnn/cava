...
void transform_fxp(float *input, int row_size, int col_size, int chan_size,
                    float *result, float TsTw_tran[3][3]) {
  ARRAY_3D(float, _input, input, row_size, col_size);
  ARRAY_3D(float, _result, result, row_size, col_size);
      ...
      for (int chan = 0; chan < chan_size; chan++)
        if (chan == 0) {
          _result[chan][row][col] =
              max(_input[0][row][col] * TsTw_tran[0][0] +
                      _input[1][row][col] * TsTw_tran[1][0] +
                      _input[2][row][col] * TsTw_tran[2][0],
                  0);
        } else if (chan == 1) {
          _result[chan][row][col] =
              max(_input[0][row][col] * TsTw_tran[0][1] +
                  _input[1][row][col] * TsTw_tran[1][1] + 
                  _input[2][row][col] * TsTw_tran[2][1], 0);
        } else {
          max(_input[0][row][col] * TsTw_tran[0][2] +
                  _input[1][row][col] * TsTw_tran[1][2] +
                  _input[2][row][col] * TsTw_tran[2][2],
              0);
        }
}
